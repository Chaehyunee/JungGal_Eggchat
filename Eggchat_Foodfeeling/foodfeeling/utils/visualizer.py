...
from foodfeeling.decorators import backend
...
class Visualizer:
    ...
    def save_result(self, loss: Tensor, eval_dict: dict, mode: str, feeling: str):
        """
        ...
        """
        if mode == 'train' or mode == 'multi-train':
            ...
            self.__save_txt(self.train_accuracy, 'train_accuracy', feeling)
            self.__save_txt(self.train_precision, 'train_precision', feeling)
            self.__save_txt(self.train_recall, 'train_recall', feeling)
            self.__save_txt(self.train_f1_score, 'train_f1_score', feeling)
            self.__save_txt(self.train_loss, 'train_loss', feeling)

        elif mode == 'test' or mode == 'multi-test':
            ...
            self.__save_txt(self.test_accuracy, 'test_accuracy', feeling)
            self.__save_txt(self.test_precision, 'test_precision', feeling)
            self.__save_txt(self.test_recall, 'test_recall', feeling)
            self.__save_txt(self.test_f1_score, 'test_f1_score', feeling)
            self.__save_txt(self.test_loss, 'test_loss', feeling)

        else:
            raise Exception('mode는 train과 test만 가능합니다.')

    def draw_graphs(self, feeling: str):
        """
        다양한 메트릭 그래프를 그립니다.
        test가 True인 경우 testing 결과도 함께 그립니다.
        """

        plt.plot(self.__load_txt('train_accuracy', feeling=feeling), 'darkgreen', label='train_accuracy')
        if len(self.test_accuracy) != 0:
            plt.plot(self.__load_txt('test_accuracy', feeling=feeling), 'limegreen', label='test_accuracy')
        self.__draw_graph('accuracy', feeling)

        plt.plot(self.__load_txt('train_precision', feeling=feeling), 'darkcyan', label='train_precision')
        if len(self.test_precision) != 0:
            plt.plot(self.__load_txt('test_precision', feeling=feeling), 'cyan', label='test_precision')
        self.__draw_graph('precision', feeling)

        plt.plot(self.__load_txt('train_recall', feeling=feeling), 'darkred', label='train_recall')
        if len(self.test_recall) != 0:
            plt.plot(self.__load_txt('test_recall', feeling=feeling), 'red', label='test_recall')
        self.__draw_graph('recall', feeling)

        plt.plot(self.__load_txt('train_f1_score', feeling=feeling), 'darkmagenta', label='train_f1_score')
        if len(self.test_f1_score) != 0:
            plt.plot(self.__load_txt('test_f1_score', feeling=feeling), 'magenta', label='test_f1_score')
        self.__draw_graph('f1_score', feeling)

        plt.plot(self.__load_txt('train_loss', feeling=feeling), 'darkgoldenrod', label='train_loss')
        if len(self.test_loss) != 0:
            plt.plot(self.__load_txt('test_loss', feeling=feeling), 'gold', label='test_loss')
        self.__draw_graph('loss', feeling)

    def draw_matrix(self, cm: np.ndarray, target_names: list, mode: str, feeling: str):
        """
        ...
        """
        np.seterr(invalid='ignore') # RuntimeWarning 숨기기
        ...
        plt.tight_layout()
        plt.savefig(self.model_file + '_confusion_matrix_{mode}_{feeling}.png'.format(mode=mode, feeling=feeling))
        plt.close()

    def draw_report(self, report: DataFrame, mode: str, feeling: str):
        """
        ...
        fig = ax.get_figure()
        fig.savefig(self.model_file + '_report_{mode}_{feeling}.png'.format(mode=mode, feeling=feeling))
        plt.close()
    ...
    def __draw_graph(self, mode: str, feeling: str):
        """
        ...
        plt.xlabel('epochs')
        plt.ylabel(mode)
        plt.title('train test {mode} {feeling}'.format(mode=mode, feeling=feeling))
        plt.grid(True, which='both', axis='both')
        plt.legend()
        plt.savefig(self.model_file + '_graph_{mode}_{feeling}.png'.format(mode=mode, feeling=feeling))
        plt.close()
    ...
    def __load_txt(self, mode: str, feeling: str):
        """
        ...
        f = open(self.model_dir + 'temp{_}{mode}_{feeling}.txt'.format(_=self.delimeter, mode=mode, feeling=feeling), 'r')
        file = f.read()
        file = re.sub('\\[', '', file)
        file = re.sub('\\]', '', file)
        f.close()
    ...
    def __save_txt(self, array: list, mode: str, feeling: str):
        """
        ...
        f = open(self.model_dir + 'temp{_}{mode}_{feeling}.txt'.format(_=self.delimeter, mode=mode, feeling=feeling), 'w')
        f.write(str(array))
        f.close()
