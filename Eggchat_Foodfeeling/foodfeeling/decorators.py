import os
import sys
sys.path.append('/data/EggChat/foodfeeling/demo')

import torch
from torch import Tensor, nn
import foodfeeling_config as config
...
def tagging(cls):
    cls = backend(cls)
    for key, val in config.TAGGING.items():
        setattr(cls, key, val)
    return cls
...
#food-feeling classifier
def food_feeling(cls):
    cls = backend(cls)
    for key, val in config.FOODFEELING.items():
        setattr(cls, key, val)
    return cls