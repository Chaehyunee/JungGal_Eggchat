...
from kochat.decorators import food_entity
...
@food_entity
class FoodFeelingEntityRecognizer(TorchProcessor):
    ...
    def __init__(self, model: nn.Module, loss: BaseLoss):
        ...
        """
        self.device = torch.device("cuda:0") # gpu 사용을 위해 추가
    ...
    def _forward(self, feats: Tensor, labels: Tensor = None, length: Tensor = None):
        ...
        if isinstance(self.loss, CRFLoss):
            ...
            predicts = torch.tensor(self.loss.decode(logits)).to(self.device)
        else:
        ...
        if labels is None:
            ...
            loss = self.loss.compute_loss(labels, logits, feats, mask).to(self.device)
