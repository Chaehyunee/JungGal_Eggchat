...
class FoodnameSearcher():
    ...
    def search_foodname(self, foodname: str, x: float, y: float) -> dict:
        """
        DB랑 통신하는 방법 찾아서
        data_dict에 담고 반환
        ...
        try:
            ...
            sql = "select share_post_id, share_post_name, share_story, share_post_region, share_time  \
                from share_post where ST_Distance_Sphere(share_post_point, ST_GeomFromText('point({} {})')) <=20000 and share_post_name LIKE '%{}%';".format(x, y, foodname)
