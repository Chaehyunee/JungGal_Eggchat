...
from foodfeeling.decorators import tagging
...
class Organizer:
    ...
    def organize_feeling_tag(self) -> dict:
        """
        ...
        # files = os.listdir(self.feeling_tag_data_dir)
        ...
        #     feeling_file = self.__process_tagging_file(file_name)
        #     integrated_file += feeling_file
        #     # 개별 파일 단위 프로세싱 이후 하나의 파일로 통합

        file = pd.read_csv(self.feeling_tag_data_dir, encoding='utf-8')

        feeling_df = DataFrame(file, columns=['FOODNAME','good','sad','tired','stress'])
        feeling_df.to_csv(self.feeling_tag_data_dir, index=False)
        feeling_dict = self.__make_feeling_dict(feeling_df['good'])
        return feeling_dict
    ...
    # def __process_tagging_file(self, file_name: str) -> list:## TODO
    #     """
    ...
    #     file = pd.read_csv(self.feeling_tag_data_dir, encoding='utf-8')
    #     questions = file['FOODNAME'].values.tolist()  # question만 뽑아서 리스트로
    ...
    def __make_feeling_dict(self, feelings: Series) -> dict:
        """
        학습 등에 사용되는 인텐트 라벨 딕셔너리를 생성합니다.

        :param feelings: 원본 데이터에서 대표적으로 good 칼럼만 뽑아낸 리스트
        :return: tag 딕셔너리 (e.g. {0:1, 1:2})
        """

        label_dict, index = {}, -1

        # 딕셔너리에 없는 feeling tag 새로 추가
        for feeling in feelings:
            if feeling not in label_dict:
                index += 1

            label_dict[feeling] = feeling
