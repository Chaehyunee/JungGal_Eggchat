...
root_dir = os.path.abspath('/data/EggChat/foodfeeling/demo')
...
DATA = {
    ...
    'feeling_tag_data_dir': BASE['root_dir'] + "data{_}tagging{_}feeling_tag_data.csv".format(_=_),  # foood-feeling 데이터 파일 경로
    'entity_data_dir': BASE['root_dir'] + "data{_}feeling_entity_data.csv".format(_=_),  # 생성된 엔티티 데이터 파일 경로

    'NER_categories': ['COMMENT'],  # 사용자 정의 태그
    'NER_tagging': ['B', 'E', 'S'],  # NER의 BEGIN, END, INSIDE, SINGLE 태그
...
TAGGING = {
    ...
    'raw_data_dir': BASE['root_dir'] + "data{_}raw{_}".format(_=_),  # 원본 데이터 파일 경로
    'ood_data_dir': BASE['root_dir'] + "data{_}ood{_}".format(_=_),  # out of distribution 데이터셋
    'feeling_tag_data_dir': BASE['root_dir'] + "data{_}tagging{_}feeling_tag_data.csv".format(_=_),  # food-feeling 데이터 파일 경로
    'entity_data_dir': BASE['root_dir'] + "data{_}feeling_entity_data.csv".format(_=_),  # 생성된 엔티티 데이터 파일 경로

    'NER_categories': ['COMMENT'],  # 사용자 정의 태그
    'NER_tagging': ['B', 'E', 'S'],  # NER의 BEGIN, END, SINGLE 태그
    'NER_outside': 'O',  # NER의 O태그 (Outside를 의미)
}
...
FOODFEELING = {
    'model_lr': 1e-4,  # 음식 기분 분류 학습시 사용되는 러닝레이트
    'loss_lr': 1e-2,  # 음식 기분 분류 학습시 사용되는 러닝레이트
    'weight_decay': 1e-4,  # 음식 기분 분류 학습시 사용되는 가중치 감쇠 정도
    'epochs': 20,  # 음식 기분 분류 학습 횟수
    'd_model': 512,  # 음식 기분 분류 모델의 차원
    'd_loss': 1,  # 음식 기분 분류 로스의 차원 (시각화차원, 높을수록 ood 디텍션이 정확해지지만 느려집니다.)
    'layers': 1,  # 음식 기분 분류 모델의 히든 레이어(층)의 수
    'grid_search': True,  # KNN과 Fallback Detector 학습시 그리드 서치 여부

    'masking': True,  # loss 계산시 패딩 마스크 여부


    'hidden_dim' : 256, # GRU - hidden 차원 값
    'embed_dim' : 128, # GRU - 임베딩 된 단어 텐서가 갖는 차원 값(dimension)
    'dropout_p' : 0.2, # GRU - Dropout 비율
    'n_classes' : 1, # GRU - 분류할 클래스 수 (good, sad, tired, stress) 총 4개 -> 각 감정에 따라서 True, False만 구분하면 되게끔 바꿈
    'n_layers' : 1, # GRU - 일반적으로는 2

    'classes' : 1, # 분류할 클래스 수 -> 각 감정에 따라서 모델 따로 만듦
