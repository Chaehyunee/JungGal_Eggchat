...
class Dataset:
    ...
    def load_intent(self, emb_processor: BaseProcessor) -> tuple:
        ...
        # intent_train, intent_test = [], [] # TODO 체크하세요
    ...
    def __embedding(self, dataset: list, emb_processor: BaseProcessor) -> tuple:
        ...
        for i, (question, label) in enumerate(dataset):
            ...
            question_list.append(question.unsqueeze(0).cpu())
            label_list.append(torch.tensor(label).cpu())
            length_list.append(torch.tensor(length).unsqueeze(0).cpu())
            # question_list.append(question.unsqueeze(0).to(self.device))
            # label_list.append(torch.tensor(label).to(self.device))
            # length_list.append(torch.tensor(length).unsqueeze(0).to(self.device))
