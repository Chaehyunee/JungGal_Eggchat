...
class Scenario:
    ...
    def __check_api(self, api):
        ...
        for entity in zip(parameters, dict_keys):
            ...
            if dict_key.lower() not in pre_defined_entity:
                raise Exception('\n\n'
                                'EggChat은 최대한 정확한 기능 수행을 위해 Config값에 정의된 Entity만 허용합니다. \n'
                                '- config에 정의된 엔티티 : {0}\n'
                                '- 시나리오 엔티티 : {1}\n'
            ...
            if api_param.lower() != dict_key.lower():
                raise Exception('\n\n'
                                'EggChat은 최대한 정확한 기능 수행을 위해 API의 파라미터의 이름과 순서를 고려하여 엔티티와 맵핑합니다.\n'
    ...
    def apply(self, entity, text, x, y, user_id):
        ...
        if len(required_entity) == 0:
            if self.intent == 'feeling':
                result_entity = "".join([*result.values()])
                return {
                    'answer': self.api(result_entity, x, y),
                    'intent': self.intent,
                    'state': 'SUCCESS'
                }

            elif self.intent == 'foodname':
                result_entity = " ".join([*result.values()])
                return {
                    'answer': self.api(result_entity, x, y),
                    'intent': self.intent,
            ...
            elif self.intent == 'food_feeling':
                result_entity = " ".join([*result.values()])
                return {
                    'answer': self.api(*result.values(), user_id),
                    'intent': self.intent,
                    'state': 'SUCCESS'
                }
